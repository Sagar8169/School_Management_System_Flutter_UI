...
class _BusTrackingPageState extends State<BusTrackingPage> {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: BottomNavigationBar(
        ...
        items: const [
          ...
          BottomNavigationBarItem(icon: Icon(Icons.directions_bus_filled), label: "Bus"),
        ...
        children: [
          ...
          const Column(
            ...
            children: [
              ...
              Text("Powered by Toki Tech", style: TextStyle(color: Color(0xFF9E9E9E), fontSize: 11)),
            ],
          ),
          const Spacer(),
          GestureDetector(
  ...
  Widget _buildHeroSection() {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // ✅ BACK BUTTON ADDED HERE (ORIGINAL LOGIC UNTOUCHED)
          Row(
            children: [
              GestureDetector(
              ...
              const SizedBox(width: 12),
              const Text("Bus Tracking", style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w700)),
            ],
          ),
          const SizedBox(height: 20),
          Container(
            ...
            decoration: BoxDecoration(color: _darkOrangeCard, borderRadius: BorderRadius.circular(20)),
            child: Column(
              children: [
                Row(
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text("Bus Route 5", style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
                          Text("KA-01-AB-1234 • Ramesh", style: TextStyle(color: Colors.white70, fontSize: 12)),
                        ],
                      ),
                    ),
                    const Icon(Icons.wifi_tethering, color: Colors.greenAccent, size: 20),
                  ],
                ),
                const Padding(
  ...
  Widget _buildToggleButtons() {
    return Padding(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            _toggleBtn("Live Tracking", _isLiveTracking, () => setState(() => _isLiveTracking = true)),
            _toggleBtn("Trip History", !_isLiveTracking, () => setState(() => _isLiveTracking = false)),
          ],
        ),
      ),
    );
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: isSelected ? [BoxShadow(color: _headerOrange.withOpacity(0.3), blurRadius: 8)] : [],
          ),
          child: Center(
            child: Text(title, style: TextStyle(color: isSelected ? Colors.white : _textSecondary, fontWeight: FontWeight.w600, fontSize: 14)),
          ),
        ),
      ),
    );
  ...
  Widget _buildLiveTrackingView() {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text("Route Progress", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
          const SizedBox(height: 20),
          _timelineStop("School Gate", "7:30 AM", status: "passed"),
          _timelineStop("Sector 11 Park", "7:42 AM", status: "current"),
          _timelineStop(_userStop, "8:00 AM", status: "future", isUserStop: true),
  ...
  Widget _timelineStop(String name, String time, {String status = "future", bool isUserStop = false, bool isLast = false}) {
    Color dotColor = status == "passed" ? _greenBadgeText : (status == "current" ? _headerOrange : Colors.grey.shade300);
          ...
          Column(
            children: [
              Container(
                width: 22, height: 22,
                decoration: BoxDecoration(
                  color: dotColor, shape: BoxShape.circle, border: Border.all(color: Colors.white, width: 4),
                  boxShadow: [if(status == "current" || isUserStop) BoxShadow(color: dotColor.withOpacity(0.4), blurRadius: 10, spreadRadius: 2)],
                ),
                child: status == "passed" ? const Icon(Icons.check, size: 10, color: Colors.white) : null,
              ),
              if (!isLast) Expanded(child: VerticalDivider(color: Colors.grey.shade300, thickness: 2, width: 22)),
            ],
          ),
          ...
          Expanded(
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: isUserStop ? _userStopBorder : Colors.grey.shade100),
              ),
              child: Row(
                children: [
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(name, style: TextStyle(fontWeight: FontWeight.w700, fontSize: 14, color: isUserStop ? _userStopIcon : _textPrimary)),
                        Text(isUserStop ? "Registered Stop for $_userChildName" : "Bus Stop", style: TextStyle(color: _textSecondary, fontSize: 11)),
                      ],
                    ),
                  ),
                  Text(time, style: TextStyle(fontWeight: FontWeight.bold, color: _textPrimary, fontSize: 13)),
                ],
              ),
            ),
          ),
  ...
  Widget _buildHistoryView() {
    return Padding(
      ...
      child: Column(
        children: [
          _historyCard("Pickup", "Nov 9, 2025", "7:30 AM - 8:15 AM"),
          _historyCard("Drop", "Nov 8, 2025", "3:00 PM - 3:45 PM"),
          _historyCard("Pickup", "Nov 8, 2025", "7:30 AM - 8:15 AM"),
        ],
      ),
    );
  }

  Widget _historyCard(String type, String date, String time) {
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Icon(type == "Pickup" ? Icons.login : Icons.logout, color: badgeColor, size: 20),
          ),
          const SizedBox(width: 16),
          Expanded(
            child: Column(
          ...
          Container(
            ...
            child: Text("Done", style: TextStyle(color: _greenBadgeText, fontSize: 10, fontWeight: FontWeight.bold)),
          )
        ],
      ),
    );
