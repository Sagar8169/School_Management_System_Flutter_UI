...
class _UploadGradePageState extends State<UploadGradePage> {
  ...
  void _submitGradesLogic() {
    ...
    bool isIncomplete =
        _students.any((s) => s['grade'].toString().trim().isEmpty);
    if (isIncomplete) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
        ...
        children: [
          Row(
            children: [
              Expanded(
                  child: InkWell(
                      onTap: () => _showPicker("Class", _classes, selectedClass,
          ...
          InkWell(
            onTap: () => _showPicker("Exam Type", _examTypes, selectedExamType,
                (val) => setState(() => selectedExamType = val)),
            child: _selectorTile(
                "Exam Type", selectedExamType, Icons.assignment_outlined),
          ),
        ],
      ),
    );
