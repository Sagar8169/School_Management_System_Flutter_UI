...
class _HomeworkDetailsPageState extends State<HomeworkDetailsPage> {
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const Text("Homework Detail", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
          const Spacer(),
          GestureDetector(
  ...
  Widget _buildMainHeroCard(Color statusCol) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(widget.homeworkData['title'], style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: Color(0xFF1E293B))),
          ...
          _infoRow(Icons.school_outlined, "Class", widget.homeworkData['class']),
          const SizedBox(height: 12),
          _infoRow(Icons.calendar_today_outlined, "Assigned Date", widget.homeworkData['assignedDate']),
          const SizedBox(height: 12),
          _infoRow(Icons.event_busy_outlined, "Due Date", widget.homeworkData['dueDate']),
        ],
      ),
    );
  }
  ...
  Widget _buildDescriptionBox() {
    return Container(
      ...
      child: Text(widget.homeworkData['description'] ?? "No description available.", style: const TextStyle(color: Color(0xFF475569), height: 1.6, fontSize: 14)),
    );
  }

  Widget _buildAttachmentsList() {
    return Column(
      children: List.generate(widget.homeworkData['attachments'], (index) => Container(
        margin: const EdgeInsets.only(bottom: 10),
        padding: const EdgeInsets.all(12),
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16)),
        child: Row(
          children: [
            const CircleAvatar(backgroundColor: Color(0xFFE3F2FD), child: Icon(Icons.picture_as_pdf_rounded, color: Colors.blue, size: 20)),
            const SizedBox(width: 12),
            const Expanded(child: Text("Attachment.pdf", style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13))),
  ...
  Widget _badge(String txt, Color col) => Container(
    ...
    child: Text(txt, style: TextStyle(color: col, fontSize: 11, fontWeight: FontWeight.w800)),
  );
  ...
  Widget _infoRow(IconData icon, String label, String val) => Row(
    children: [
      ...
      Text(val, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Color(0xFF334155))),
    ],
  );

  Widget _statItem(String val, String label, Color col) => Column(
    children: [
      Text(val, style: TextStyle(color: col, fontSize: 24, fontWeight: FontWeight.w900)),
  ...
  Widget _buildSectionLabel(String text) => Padding(
    ...
    child: Text(text.toUpperCase(), style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w900, color: Colors.grey, letterSpacing: 1.2)),
  );
