...
class _ClassDetailsPageState extends State<ClassDetailsPage> {
  ...
  final List<Map<String, dynamic>> _performanceData = [
    ...
    {'category': 'Needs Attention (Below 40%)', 'students': 1, 'color': const Color(0xFFEF4444)},
  ];
  ...
  void _showTimetableMessage() {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Timetable for ${widget.classData['className'] ?? 'Class'}'),
  ...
  void _onBottomNavTapped(int index) {
    ...
    switch (index) {
      case 0: Navigator.of(context).pushNamedAndRemoveUntil(SubjectTeacherRoutes.home, (route) => false); break;
      case 1: Navigator.pushNamed(context, SubjectTeacherRoutes.search); break;
      case 2: _navigateTo(SubjectTeacherRoutes.activity); break;
      case 3: _navigateTo(SubjectTeacherRoutes.settings); break;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final String classShort = className.contains(' ') ? className.split(' ').last : className;
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildHeroDashboard(className, section, teacher, classShort),
                      const SizedBox(height: 16),
                      _buildQuickStats(),
                      ...
                      const SizedBox(height: 32),
                      _buildOriginalButtons(className), // ✅ Same 3 Buttons from original
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const Expanded(child: Text("Class Details", style: TextStyle(color: Color(0xFF1E293B), fontSize: 18, fontWeight: FontWeight.bold))),
          GestureDetector(
  ...
  // --- HERO CARD ---
  Widget _buildHeroDashboard(String name, String sec, String teacher, String short) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(24),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [_primaryPurple, _darkPurple], begin: Alignment.topLeft, end: Alignment.bottomRight),
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child: Center(child: Text(short, style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold))),
              ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text("Section ${sec.replaceAll('Section ', '')}", style: TextStyle(color: Colors.white.withOpacity(0.8), fontSize: 14)),
                  ],
                ),
              ),
            ],
          ...
          Row(
            children: [
              ...
              Text("Class Teacher: ", style: TextStyle(color: Colors.white.withOpacity(0.7), fontSize: 14)),
              Text(teacher, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14)),
            ],
          ),
        ],
      ),
  ...
  Widget _statBox(String v, String l, Color c) => Container(
    ...
    child: Column(
      children: [
        ...
        Text(l, style: const TextStyle(fontSize: 10, color: Colors.blueGrey, fontWeight: FontWeight.bold)),
      ],
    ),
  );
  ...
  Widget _buildAnalyticsCard() {
    return Container(
      padding: const EdgeInsets.all(24),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(28), border: Border.all(color: Colors.black.withOpacity(0.03))),
      child: Column(
            ...
            child: Column(
              children: [
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text(data['category'], style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Color(0xFF334155))),
                    Text("${data['students']} students", style: TextStyle(color: data['color'], fontWeight: FontWeight.bold, fontSize: 13)),
                  ],
                ),
                const SizedBox(height: 10),
                ClipRRect(
                  borderRadius: BorderRadius.circular(10),
                  child: LinearProgressIndicator(value: percent, backgroundColor: const Color(0xFFF1F5F9), valueColor: AlwaysStoppedAnimation<Color>(data['color']), minHeight: 8),
                ),
              ],
            ),
          );
        ...
        children: [
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text(s['teacher'], style: const TextStyle(color: Colors.grey, fontSize: 12)),
            ]),
          ),
          Column(
            crossAxisAlignment: CrossAxisAlignment.end,
            children: [
              const Text("Avg Score", style: TextStyle(fontSize: 10, color: Colors.grey)),
              Text("${s['score']}%", style: TextStyle(color: _primaryPurple, fontWeight: FontWeight.bold, fontSize: 18)),
            ],
          ),
        ],
      ),
  ...
  Widget _buildOriginalButtons(String className) {
    return Column(
      children: [
        _btn("View All Students", Icons.people_alt_outlined, _primaryPurple, true, () {
          _navigateTo(SubjectTeacherRoutes.studentSearch, arguments: {'classFilter': className});
        }),
        const SizedBox(height: 12),
        _btn("View Timetable", Icons.calendar_today_rounded, _primaryPurple, false, () => _showTimetableMessage()),
        const SizedBox(height: 12),
        _btn("View Attendance and Grades History", Icons.history_rounded, _primaryPurple, false, () {
          _navigateTo(SubjectTeacherRoutes.attendanceAnalytics, arguments: {'classData': widget.classData});
        }),
      ],
    );
  }

  Widget _btn(String t, IconData i, Color c, bool fill, VoidCallback onTap) => SizedBox(
    width: double.infinity, height: 56,
    child: ElevatedButton.icon(
      onPressed: onTap, icon: Icon(i, size: 20), label: Text(t),
      style: ElevatedButton.styleFrom(
        backgroundColor: fill ? c : Colors.white, foregroundColor: fill ? Colors.white : c,
        elevation: 0, side: fill ? null : BorderSide(color: c.withOpacity(0.4)),
  ...
  Widget _sectionTitle(String title) => Padding(
    ...
    child: Text(title, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w900, color: Color(0xFF1E293B))),
  );
