...
class _TakeAttendancePageState extends State<TakeAttendancePage> {
  ...
  final List<String> _classList = ['Class 8A', 'Class 8B', 'Class 9A', 'Class 9B', 'Class 10A'];
  final List<String> _sectionList = ['Section A', 'Section B', 'Section C', 'Section D'];
              ...
              ConstrainedBox(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final bool isSelected = isClass ? selectedClass == item : selectedSection == item;
                    return GestureDetector(
                      onTap: () {
                        setState(() {
                          if (isClass) selectedClass = item;
  ...
  void _submitAttendanceLogic() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text("Attendance Submitted Successfully!", textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.bold)),
        actions: [
          Center(child: TextButton(onPressed: () => Navigator.pop(context), child: Text("OK", style: TextStyle(color: _primaryPurple, fontWeight: FontWeight.bold)))),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    int presentCount = studentAttendance.values.where((v) => v == 'Present').length;
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const Text("Students List", style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: Color(0xFF1E293B))),
                    const SizedBox(height: 12),
                    _buildStudentList(),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const Text("Take Attendance", style: TextStyle(color: Color(0xFF1E293B), fontSize: 18, fontWeight: FontWeight.bold)),
          const Spacer(),
          GestureDetector(
  ...
  Widget _buildModernClassSelector() {
    return Container(
      ...
      child: Row(
        children: [
          Expanded(child: InkWell(onTap: () => _showPicker("Class", _classList, true), borderRadius: const BorderRadius.horizontal(left: Radius.circular(24)), child: _selectorTile("Class", selectedClass, Icons.school_outlined))),
          Container(height: 40, width: 1, color: Colors.grey[200]),
          Expanded(child: InkWell(onTap: () => _showPicker("Section", _sectionList, false), borderRadius: const BorderRadius.horizontal(right: Radius.circular(24)), child: _selectorTile("Section", selectedSection, Icons.layers_outlined))),
        ],
      ),
    );
  }
  ...
  Widget _selectorTile(String l, String v, IconData i) => Padding(
    ...
    child: Column(children: [
      ...
      Row(mainAxisAlignment: MainAxisAlignment.center, children: [Text(v, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 16)), const Icon(Icons.keyboard_arrow_down_rounded, color: Colors.grey, size: 18)]),
    ]),
  );
  ...
  Widget _statBox(String l, String v, Color c) => Expanded(
    child: Container(
      ...
      child: Column(children: [Text(v, style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: c)), Text(l, style: TextStyle(fontSize: 12, color: c, fontWeight: FontWeight.bold))]),
    ),
  );
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return Container(
          ...
          child: Row(
            children: [
              CircleAvatar(radius: 16, backgroundColor: _bgLight, child: Text(student['id'].toString(), style: const TextStyle(fontSize: 10, color: Colors.grey))),
              const SizedBox(width: 12),
  ...
  Widget _attBtn(int id, String l, bool active) {
    ...
    return GestureDetector(
      onTap: () => setState(() => studentAttendance[id] = l == 'P' ? 'Present' : 'Absent'),
      child: AnimatedContainer(duration: const Duration(milliseconds: 200), width: 40, height: 40, decoration: BoxDecoration(color: active ? c : Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: active ? c : Colors.grey.shade300)), child: Center(child: Text(l, style: TextStyle(color: active ? Colors.white : Colors.grey, fontWeight: FontWeight.bold)))),
    );
  }
  ...
  Widget _buildSubmitButton() {
    return SizedBox(
      ...
      child: ElevatedButton(onPressed: _submitAttendanceLogic, style: ElevatedButton.styleFrom(backgroundColor: _primaryPurple, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)), elevation: 4), child: const Text('Submit Attendance', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold))),
    );
  }
