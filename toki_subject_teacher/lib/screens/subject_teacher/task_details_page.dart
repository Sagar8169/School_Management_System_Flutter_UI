...
class _TaskDetailsPageState extends State<TaskDetailsPage> {
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const Text("Description", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Color(0xFF1E293B))),
                    ...
                    const Text("Assigned By", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Color(0xFF1E293B))),
                    const SizedBox(height: 12),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const Text("Task Details", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
          const Spacer(),
          GestureDetector(
  ...
  Widget _buildMainInfoCard(Color statusBg, Color statusTxt) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(widget.taskData['title'], style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w900, color: Color(0xFF1E293B))),
          const SizedBox(height: 16),
          Row(
            children: [
              _statusBadge(widget.taskData['status'], statusBg, statusTxt),
              const SizedBox(width: 8),
              _statusBadge(widget.taskData['tag'], const Color(0xFFE8EAF6), const Color(0xFF3949AB)),
            ],
          ),
          const Padding(padding: EdgeInsets.symmetric(vertical: 20), child: Divider(height: 1)),
          _buildDetailRow(Icons.class_outlined, "Class", widget.taskData['class'], const Color(0xFFE8EAF6), const Color(0xFF3949AB)),
          const SizedBox(height: 16),
          _buildDetailRow(Icons.assignment_turned_in_outlined, "Submissions", widget.taskData['submissions'], const Color(0xFFE8F5E9), const Color(0xFF4CAF50)),
          const SizedBox(height: 16),
          _buildDetailRow(Icons.calendar_today_rounded, "Due Date", widget.taskData['dueDate'], const Color(0xFFFCE4EC), const Color(0xFFE91E63)),
        ],
      ),
    );
  }
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(widget.taskData['assignedBy'] ?? "Admin", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: Color(0xFF1E293B))),
                Text(widget.taskData['assigneeRole'] ?? "Staff", style: const TextStyle(color: Colors.grey, fontSize: 12, fontWeight: FontWeight.w500)),
              ],
            ),
          ),
        ],
  ...
  Widget _statusBadge(String txt, Color bg, Color txtCol) => Container(
    ...
    child: Text(txt, style: TextStyle(color: txtCol, fontSize: 11, fontWeight: FontWeight.w800)),
  );
  ...
  Widget _buildContentCard({required Widget child}) => Container(
    ...
    padding: const EdgeInsets.all(20),
    decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(24), border: Border.all(color: Colors.black.withOpacity(0.01))),
    child: child,
  );
